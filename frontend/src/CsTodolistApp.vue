  ...
  <div class="left-off-canvas-menu" :class="{ 'active': showLeftMenu }">
    <div class="left-menu-inner">
      ...
      <nav class="left-nav-links">
        ...
        <a href="cs_todolist.html" class="nav-link active"><span class="icon">📋</span>해야할일</a>
...
const columns = [
  { id: 'pending', title: '대기중', color: '#f59e0b', placeholder: '+ 할일 입력 후 Enter' },
...
const newCardTitles = ref({ pending: '', in_progress: '', done: '' })
...
ALTER TABLE todo_cards ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Allow all" ON todo_cards
...
async function loadCards() {
  ...
  try {
    if (usingSupabase.value) {
      ...
      const saved = localStorage.getItem('todo_cards')
...
function saveLocal() {
  localStorage.setItem('todo_cards', JSON.stringify(allCards.value))
}
...
async function saveEdit(card) {
  ...
  if (newTitle === card.title && newDescription === (card.description || '')) { cancelEdit(); return }
...
function onDragStart(cardId) { dragCardId.value = cardId }
function onDragOver(colId) { dragOverColumn.value = colId }
...
function onDrop(colId) {
  ...
  if (card && card.status !== colId) moveCard(card, colId)
  dragCardId.value = null
}
function onDragEnd() { dragCardId.value = null; dragOverColumn.value = null }
...
.sql-block {
  text-align: left; background: #1e293b; color: #e2e8f0;
...
.kanban-card {
  ...
  display: flex; flex-direction: column; gap: 6px; min-height: 42px;
}
...
.card-title { font-size: 14px; color: #1f2937; line-height: 1.4; word-break: break-word; }
.card-description { font-size: 13px; color: #6b7280; line-height: 1.4; margin-top: 2px; }
.card-note {
  font-size: 12px; color: #3b82f6; line-height: 1.4; margin-top: 2px;
  background: #eff6ff; padding: 4px 8px; border-radius: 4px;
}
.card-date { font-size: 11px; color: #9ca3af; margin-top: 4px; }
.card-actions {
...
.card-btn {
  background: none; border: none; cursor: pointer; font-size: 16px;
  padding: 2px 6px; border-radius: 4px; color: #9ca3af; line-height: 1;
}
...
.card-edit-input + .card-edit-input { margin-top: 6px; }
.edit-area { display: flex; flex-direction: column; width: 100%; }
.note-area { margin-top: 6px; padding-top: 6px; border-top: 1px solid #e5e7eb; }
.note-input {
  width: 100%; border: 1px solid #e5e7eb; border-radius: 6px;
  padding: 6px 8px; font-size: 12px; font-family: inherit;
  resize: none; outline: none; line-height: 1.4; color: #6b7280; background: #f9fafb;
}
.note-input:focus { border-color: #3b82f6; background: white; }
.add-card-area { display: flex; gap: 4px; padding: 4px; }
.add-card-input {
